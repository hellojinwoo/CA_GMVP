...
if __name__ == "__main__":
    ...
    daily_return_df_list_val, daily_return_df_list_test = prepare_data.split_dataset(file_path)
    ...
    validation_investment_date = prepare_data.daily_price_df.loc[validation_first_date:validation_last_date,:].index

    test_first_date =  str(daily_return_df_list_test[1].index[-60].date())
    test_last_date = str(daily_return_df_list_test[-1].index[-1].date())
    test_investment_date_validation = prepare_data.daily_price_df.loc[test_first_date:test_last_date,:].index
